...
3. Compile the slides: `typst compile introduction.typ` → creates a new pdf named `introduction.pdf` in the same folder. If you use the executable file `typst-x86_64-unknown-linux-musl`, make sure to include the full path, e.g. `~/code/tools/typst-x86_64-unknown-linux-musl compile introduction.typ`.
